...
local AdamOptimizer, parent = torch.class("dp.AdamOptimizer", "dp.Propagator")
AdamOptimizer.isAdamOptimizer = true

function AdamOptimizer:__init(config)
   config = config or {}
...
function AdamOptimizer:setup(config)
   parent.setup(self, config)
   self._model:zeroGradParameters() -- don't forget this, else NaN errors
end
      
function AdamOptimizer:propagateBatch(batch, report)
   function feval(params)
      self._model:training()
      self:forward(batch)
      self:monitor(batch, report)
      self:backward(batch)
      return self.err, gradParams
   end
   if report.epoch % self._update_interval == 0 then
      self._callback(feval, self._model, report)
   end
   self:doneBatch(report)
end

function AdamOptimizer:backward(batch)
