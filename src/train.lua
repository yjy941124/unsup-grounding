...
textualModule:add(nn.LookupTableMaskZero(ds.vocab_size,opt.input_encoding_size):init('weight',nninit.xavier))
seqlstm = nn.SeqLSTM(opt.input_encoding_size, opt.rnn_size):init('weight',nninit.uniform,-0.1, 0.1)
seqlstm.batchfirst = true
seqlstm.maskzero = true
textualModule:add(seqlstm)
textualModule:add(nn.SelectLastNonZero())
textualModule:add(nn.LinearNoBias(opt.rnn_size, opt.att_size):init('weight',nninit.xavier))
...
visualModule:add(nn.LinearNoBias(opt.input_visual_size, opt.att_size):init('weight',nninit.xavier))
...
attention:add(nn.Linear(opt.att_size,1):init('weight',nninit.xavier)
                                       :init(getBias,nninit.xavier))
...
reconstruct:add(nn.Linear(opt.input_visual_size,opt.input_encoding_size):init('weight',nninit.xavier)
                                                                        :init(getBias,nninit.xavier))
